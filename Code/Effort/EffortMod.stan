...
transformed parameters {
  ...
  vector[N_tot] licenses_c = licenses - mean(licenses);
  vector[N_tot] fleet_c = fleet - mean(fleet);
}
...
model {
  ...
  beta0_E ~ normal(mu_0E, sigma_0E);
  beta1_E ~ normal(mu_1E, sigma_1E);
  ...
  mu_fleet = beta0_F[area] + beta1_F[area] .* licenses_c;
  vector[N_tot] mu_effort;
  for (n in 1:N_tot) {
    mu_effort[n] = beta0_E[combo_idx[n]] + beta1_E[combo_idx[n]] .* fleet_c[n];
  }
