...
#The modelling
Supply_mod <- cmdstan_model(stan_file = "Abundance/Supply_rickermodSSTHatchStd.stan", pedantic=T) #adds hatcheries (.91)
datalist <- list(N = nrow(dat),
                 ...
                 SST_obs = (dat$parental_sst[!is.na(dat$parental_sst)]),
                 ...
                 H_obs = (dat$parental_release[!is.na(dat$parental_release)]),
                 ...
                 mean_log_P = mean(log(dat$parental_abundance[!is.na(dat$parental_abundance)])),
                 sd_log_P = sd(log(dat$parental_abundance[!is.na(dat$parental_abundance)])),
                 mean_SST = mean(dat$parental_sst[!is.na(dat$parental_sst)]),
                 sd_SST = sd(dat$parental_sst[!is.na(dat$parental_sst)]),
                 mean_H = mean(dat$parental_release[!is.na(dat$parental_release)]),
                 sd_H = sd(dat$parental_release[!is.na(dat$parental_release)]),
                 mean_log_R = mean(log(dat$abundance[!is.na(dat$abundance)])),
                 sd_log_R = sd(log(dat$abundance[!is.na(dat$abundance)])))
...
bayes_R2 <- function(fit) {
  ...
  sigma <- posterior::as_draws_matrix(fit$draws("sigma_std"))  # Replace "sigma" with the name of your residual standard deviation parameter
  var_res <- sigma^2
...
plots <- purrr::map(unique_species, function(species_of_interest) {
...
# Loop over each species
plots <- purrr::map(unique_region, function(region_of_interest) {
  # Subset observed data, excluding NAs in price.per.kilo.wholesale
...
draws_df <- tidybayes::spread_rvars(Supply, preds[i], log_mu[i])|>
  median_qi(preds,log_mu) |> mutate(abundance = dat$abundance[i],
...
write_csv(Supply$summary(), "../Results/Supply/ModelParamsStd.csv")



## Predictions ##
Nsamp_pred <- cmdstan_model(stan_file = "Abundance/Supply_rickermodSSTHatchStd_predictionsNoNA.stan", pedantic=T)
...
newdata <- expand_grid(species=c("Pink","Chum","Coho","Chinook","Sockeye"),
                       ...
                       parental_sst = seq(from=4.724,to=7.296,length.out=reps), #c(5.689), #
                       parental_release = c(0) #seq(from=163007,to=10763799,length.out=reps) #
)
...
png(file = "../Results/Supply/SupplySSTLog.png", width = 3600, height = 2400, res = 300)
ModPreds |> 
  #gather_rvars(log_mu[i]) |> 
  gather_rvars(mu[i]) |> 
  #gather_rvars(preds[i]) |> 
  bind_cols(newdata) |> 
  #ggplot(aes(x = (parental_abundance), dist = .value)) + 
...
Nsamp_pred <- cmdstan_model(stan_file = "Abundance/Supply_rickermodSSTHatchStd_preds.stan", pedantic=T)
data <-  list(N = nrow(dat),
              ...
              ii_obs_T = which(!is.na(dat$parental_sst)),
              ii_mis_T = which(is.na(dat$parental_sst)),
              SST_new_obs = (dat$parental_sst[!is.na(dat$parental_sst)]),
              
              N_miss_H = sum(is.na(dat$parental_release)),
              N_obs_H = sum(!is.na(dat$parental_release)),
              ii_obs_H = which(!is.na(dat$parental_release)),
              ii_mis_H = which(is.na(dat$parental_release)),
              H_new_obs = (dat$parental_release[!is.na(dat$parental_release)]),
...
png(file = "../Results/Supply/AbundanceTimeSppPoints.png", width = 3400, height = 2800, res = 300)
ModPreds |> 
  gather_rvars(preds[i]) |> 
  #gather_rvars(mu[i]) |> 
  bind_cols(dat) |>
